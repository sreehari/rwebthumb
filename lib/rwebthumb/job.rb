    ...
    class Job < Base
      ...
      def self.from_status_xml(api_key, api_endpoint, xml)
        ...
        job = Job.new(api_key, api_endpoint, status_element.attributes['id'], nil, submission_datetime, 5, nil,
          status_element.text.try(:downcase) == 'complete' ? STATUS_PICKUP : STATUS_PROCESSING)
      end
      # Constructor.
      ...
      def check_status
        ...
        @status = status_elem.text.try(:downcase) == 'complete' ? STATUS_PICKUP : STATUS_PROCESSING
